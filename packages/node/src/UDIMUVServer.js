...
//         try {
//           await user.signUp();
//           socket.emit(MSG_TYPE.SIGN_UP_SUCCESS);
//         } catch (error) {
//           console.error(
...
//           socket.emit(MSG_TYPE.INFO, error.message);
//         }
//       })();
//     });
...
//           const avatarString = await parseUser.get(Constant.DB.USER.AVATAR);
//           const settingsString = await parseUser.get(Constant.DB.USER.SETTINGS);
...
//         socket.emit(MSG_TYPE.ON_AVATAR, response);
//       } catch (e) {
//         console.error(e);
//       }
//     });
...
//             try {
//               const results = await query.distinct(Constant.DB.USER.AVATAR);
//               const paths = [];
...
module.exports = class UDIMUVServer {
  constructor() {
    /** @type {ExpressAppWrapper} */
    this.expressAppWrapper = new ExpressAppWrapper();
  ...
  start(config) {
    this.expressAppWrapper
      .start({
        folder: config.folder,
        port: config.port,
        withDefaultGameSocketService: false, // do not use the default gameservicesocket of ud-viz
      })
      .then(() => {
        // launch a customize gamesocketservice
        this.gameSocketService = new Game.SocketService(
